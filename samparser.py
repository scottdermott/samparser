...
def samparse(samhive):
        ...
        if x.name() == "Names":
            ...
            for a in x.values():
                ...
                if a.name() == "V":
                    ...
                    username = bytes(data[username_ofst+0xCC:username_ofst+0xCC + username_lngth]).decode("utf-8").replace('\x00','')
                    print(username)

                    results['users'][username] = OrderedDict()

                    results['users'][username]['Full Name'] = bytes(data[(fullname_ofst+0xCC):(fullname_ofst+0xCC + fullname_lngth)]).decode("utf-8").replace('\x00','')
                    results['users'][username]['Comment'] = bytes(data[(comment_ofst+0xCC):(comment_ofst+0xCC + comment_lngth)]).decode("utf-8").replace('\x00','')
                    ...
                    results['users'][username]['Drive Letter'] = bytes(data[(driveletter_ofst+0xCC):(driveletter_ofst+0xCC + driveletter_lngth)]).decode("utf-8").replace('\x00','')
                    results['users'][username]['Profile Path'] = bytes(data[(profilepath_ofst+0xCC):(profilepath_ofst+0xCC + profilepath_lngth)]).decode("utf-8").replace('\x00','')
                    results['users'][username]['Logon Script'] = bytes(data[(logonscript_ofst+0xCC):(logonscript_ofst+0xCC + logonscript_lngth)]).decode("utf-8").replace('\x00','')
                    results['users'][username]['Workstations'] = bytes(data[(workstations_ofst+0xCC):(workstations_ofst+0xCC + workstations_lngth)]).decode("utf-8").replace('\x00','')
                    #results['users'][username]['LM Password Hash'] = bytes(data[(lmpwhash_ofset+0xCC):(lmpwhash_ofset+0xCC + lmpwhash_lngth)]).decode("utf-8").replace('\x00','')
                    #results['users'][username]['NT Password Hash'] = bytes(data[(ntpwhash_ofset+0xCC):(ntpwhash_ofset+0xCC + ntpwhash_lngth)]).decode("utf-8").replace('\x00','')

                    continue
    #Now we combine the two!
    for RID in f_values:
        for user in results['users']:
            if 'RID' in results['users'][user].keys() and results['users'][user]['RID'] == str(RID):
        ...
        for user in results:
            if 'RID' in results[user].keys() and results[user]['RID'] == str(RID):
                results['users'][user]['Last Login Date'] = f_values[RID][0]
                results['users'][user]['Password Reset Date'] = f_values[RID][1]
                results['users'][user]['Password Fail Date'] = f_values[RID][2]
                results['users'][user]['Account Flags'] = []		
    ...
    for x in groupsRoot.subkeys():
        if x.name()[:5] == "00000":			#We dont actually need the Names/Members keys here beacuse everything is in the "C" value
            ...
            groupname = bytes(data[(name_offst+52):(name_offst+52+name_length)]).decode("utf-8").replace('\x00','')
